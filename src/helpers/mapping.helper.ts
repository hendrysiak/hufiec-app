    ...
    for (const [key, value] of Object.entries(object)) {
      ...
      if (updatedValue) {
        ...
        if ("surname" in updatedValue) {
          const parsedSurname = updatedValue.surname
            ? `${updatedValue.surname}`
            : "";
          updatedValue.surname =
            parsedSurname[0]?.toUpperCase() +
            parsedSurname.substring(1)?.toLowerCase();
        }
    ...
    for (const [key, value] of Object.entries(object)) {
      ...
      if (updatedValue) {
        ...
        if ("name" in updatedValue) {
          const parsedName = updatedValue.name ? `${updatedValue.name}` : "";
          updatedValue.name =
            parsedName[0].toUpperCase() + parsedName.substring(1).toLowerCase();
        }
