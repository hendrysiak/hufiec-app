...
export function LogOut(): JSX.Element {
  ...
  const disableLogout = currentPath === '/';
    ...
    <>
      ...
      <div className={`${classes.container} ${classes.close}`}>
        <Tooltip title="Wyloguj" aria-label="log-out">
          <IconButton onClick={() => signOutFromApp()?.then(() => history.push('/'))} sx={{ color: 'white' }} size="large"><ExitToAppIcon className="clicked" /></IconButton>
        </Tooltip>
      </div>
    </>
  );
