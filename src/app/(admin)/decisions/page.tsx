...
import {
  ...
  editDecision,
  getDecisions,
} from "helpers/api-helpers/decision";
...
function DecisionContainer(props: DecisionProps) {
  ...
  const edidDecisionMutation = useMutation(editDecision, {
    onSuccess: () => {
      ...
      setSnackbar({
        children: "Decyzja wyedytowana pomyślnie",
        severity: "success",
      });
    },
    onError: () => {
      setSnackbar({
        children: "Wystąpił błąd przy edytowaniu decyzji",
        severity: "error",
      });
    },
  });
  ...
  const deleteProposalMutation = useMutation(deleteDecision, {
    onSuccess: () => {
      queryClient.invalidateQueries("decision");
      setSnackbar({
        children: "Decyzja usunięta pomyślnie",
        severity: "success",
      });
    },
    onError: () => {
      setSnackbar({
        children: "Wystąpił błąd przy usuwaniu decyzji",
        severity: "error",
      });
    },
  });
  ...
  const handleCellEditCommit = (params: GridCellEditCommitParams) => {
    ...
    const foundedDecision: Decision | undefined = query.data?.find(
      (r) => r.id === id
    );
    ...
    if (foundedDecision) {
      ...
      setSnackbar({
        children: "Wystąpił wewnętrzny błąd - spróbuj ponownie",
        severity: "error",
      });
    }
  };
    ...
    },
    {
      ...
      getActions: ({ id }: { id: string }) => {
        ...
        if (element?.area === DecisionArea.Code) {
          ...
          const teamNameToUse =
            !extendedElement.targetTeams ||
            extendedElement.targetTeams.length > 1
              ? ""
              : teamsMap.find(
                  (team) => team.teamId === extendedElement.targetTeams[0]
                )?.nameToUse;
  ...
  const generateCodeActionDescription = (decision: DecisionCode) => {
    ...
    const event = codePattern.find(
      (code) => code.value === decision.prefix
    )?.name;
  ...
  const generateReturnActionDescription = (decision: DecisionReturn) => {
    const returnsMap = decision.returnInfo.map(
      (info) => `kwota: ${info.cash} z dnia ${info.date} tytułem: ${info.title}`
    );

    return [
      `Zwrot z powodu ${decision.reason} następujących wpłat:`,
      ...returnsMap,
    ];
  };

  const generateReAccountActionDescription = (
    decision: DecisionReAccouting
  ) => {
    const reAccountMap = decision.reAccountingInfo?.map(
      (info) =>
        `kwota: ${info.cash} pismem ${info.letterNumber} na: ${info.targetCode}`
    );
