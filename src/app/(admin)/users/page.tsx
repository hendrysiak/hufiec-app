...
function Role(): JSX.Element {
  ...
  const updateUserMutation = useMutation(updateUser, {
    onSuccess: () => {
      ...
      setSnackbar({
        children: "Użytkownik zaktualizowany pomyślnie",
        severity: "success",
      });
    },
    onError: () => {
  ...
  const createUserMutation = useMutation(createUser, {
    onSuccess: () => {
      queryClient.invalidateQueries("users");
      setSnackbar({
        children: "Użytkownik dodany pomyślnie",
        severity: "success",
      });
    },
    onError: () => {
  ...
  const deleteUserMutation = useMutation(deleteUser, {
    onSuccess: () => {
      queryClient.invalidateQueries("users");
      setSnackbar({
        children: "Użytkownik usunięty pomyślnie",
        severity: "success",
      });
    },
    onError: () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderEditCell: (params: GridRenderCellParams<number[]>) => (
        <TeamMultiCheckboxesEditCell params={params} />
      ),
      width: 350,
      editable: true,
    },
  ...
  const rows = query?.data
    ? Object.entries(query.data).map(([uid, user]: [string, AuthUser]) => ({
        id: uid,
        uid: uid,
        evidenceNumber: user.evidenceNumber,
        roles: user.role, // Fixed: Changed user.role to user.roles
        ...
        >
          ...
          >
            ...
            <TextField
              ...
              onChange={(e) =>
                setNewUser({ ...user, evidenceNumber: e.target.value })
              }
          ...
          >
            <TextField
              ...
              value={user.role}
              onChange={(e) =>
                setNewUser({ ...user, role: e.target.value as UserRoles })
              }
              label="Rola"
              variant="standard"
              select
            ...
            <TextField
              ...
              value={user.team}
              onChange={(e) =>
                setNewUser({ ...user, team: [...user?.team, e.target.value] })
              }
              select
              label="Drużyna"
              variant="standard"
