...
function Team(): JSX.Element {
  ...
  useEffect(() => {
    const row = incomesByCode?.length
      ? incomesByCode.map((el, index) => ({
          ...el,
          lp: index + 1,
  ...
  useEffect(() => {
    ...
    const filteredIncomes =
      ...
      rows.filter((i) => {
        if (
          (selectedDateFrom &&
            new Date(i.dateOfBook).getTime() <= selectedDateFrom.getTime()) ||
          (selectedDateTo &&
            new Date(i.dateOfBook).getTime() >= selectedDateTo.getTime())
        )
          return false;
        if (event !== "" && i.event !== event && event !== "unAssigned")
          return false;
        if (
          event !== "" &&
          event !== "unAssigned" &&
        ...
        if (
          ...
          return false;
        if (name !== "" && !new RegExp(name, "gi").test(`${i.name}`))
          return false;
        if (surname !== "" && !new RegExp(surname, "gi").test(`${i.surname}`))
          return false;
        return true;
      });
  ...
  useEffect(() => {
    ...
    window.addEventListener("scroll", () =>
      listenerScroll(navBar, window.scrollY)
    );
    return () =>
  ...
  //               handleSelectTeam(e.target.value as string)
  //             }
  //             displayEmpty
  ...
  //       Ostatni import był:{" "}
  //       <b>
  //         {lastImportDate ? new Date(lastImportDate).toLocaleDateString() : ""}
  //       </b>
  //     </p>
