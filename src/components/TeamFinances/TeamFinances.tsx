...
function TeamFinances({
  ...
  useEffect(() => {
    ...
    const sumOfFees =
      ...
      incomes
        .filter(
          (income) => income.event === "SC" && income.year === currentYear - 1
        )
        .reduce((sum: number, income) => sum + income.cash, 0);

    const sumOfFeesForTeam =
      incomes &&
      incomes
    ...
    const sumOfCompensation = incomes
      ...
      .reduce((sum: number, income) => sum + Number(income.cash), 0);
    const sumOfOutcomes = outcomes
      .filter((o) => o.foundingSource === FoundingSources.TeamAccount)
      .reduce((sum: number, outcome) => sum + Number(outcome.cash), 0);
      ...
      <div style={{ height: "94vh" }}>
        <section className={classes.positionModal}>
          ...
          <div className={classes.item}>
            <p>STAN KONTA ZA ROK UBIEGŁY (1.5% + KONTO)</p>
            <p>{onePercent}</p>
          </div>
          <div className={classes.item}>
            <p>SKŁADKI ZEBRANE</p>
