...
const AdminActions = () => {
  ...
  return (<Paper elevation={3}>
    <Box p={4}>
      ...
      <Box pt={3.5} pb={3.5}>
        <p>Dzięki temu wyczyścisz nieaktualne przelewy, dotyczące składek</p>
      </Box>
      ...
      <Box p={4}>
        ...
        <Button variant="contained" color="secondary" onClick={() => setMapperModalOpen(true)}>Otwórz mapper</Button>
      </Box>
      <Box p={4}>
        <h2>Zaimportuj dane z Tipi</h2>
        <Box pt={3.5} pb={3.5}>
          <p>Dzięki temu zaktualizujesz bazę danych aplikacji o dane z ewidencji</p>

        </Box>
        <Button variant="contained" color="secondary" onClick={() => setTipiModalOpen(true)}>Otwórz importer</Button>
      </Box>
    </Box>
    {mapperModalOpen && <ModalMapInitAccountToRegistry open={mapperModalOpen} setOpen={setMapperModalOpen} />}
    {tipiModalOpen && <ModalTipiImport open={tipiModalOpen} setOpen={setTipiModalOpen} />}
  </Paper>)
};
